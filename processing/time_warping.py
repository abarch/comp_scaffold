...
Taken from https://github.com/deepbrook/sparseDTW/blob/master/sparse_dtw.py
"""
...
class SparseDTW:
    ...
    def __init__(self, s, q, res=.5):
        ...
        self.res = res
        self.S = self.quantize(s)
        self.Q = self.quantize(q)
    ...
    def populate_warp(self):
        ...
        while 0 <= lower_bound <= 1 - self.res / 2:
            ...
            # For indices, calculate euclidean distances and add to warp matrix SM
            for i in idxS:
                for j in idxQ:
...
if __name__ == '__main__':
    ...
    A, B = np.array([1, 2, 3, 4, 2, 3]), np.array([1, 1, 1, 1, 1, 1, 2, 3, 3, 4, 3, 3])
    dtw = SparseDTW(A, B)
